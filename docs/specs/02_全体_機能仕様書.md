...
- 必須: `id, device_id, type, val, timestamp`
- 認証: Device token（MVP）/将来JWT

### 6.2 App -> Cloud
- Endpoint: `POST /functions/v1/ingest-movement`
- 必須: `movements[]`（`id, minor, action, timestamp`）
- `action=ENTER/EXIT` は `staff_movements` に保存する。
- `action=CARE_*` は `care_records` に正規化保存する。
- `staff_id` はpayloadで受け取らず `auth.uid()` から確定する。
- 認証: User JWT
...
- 不正アクション: `ENTER/EXIT/CARE_*` 以外は `400 INVALID_ACTION`。
...
| TEST-008 | CARE正規化保存 | `CARE_*`受信時に`care_records`へ保存される |
