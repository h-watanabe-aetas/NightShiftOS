...
| NS-PLT-005 | センサー抽象化レイヤ実装 | P2 | L | NS-PLT-001 | 新規レーダー追加時にCloud/App契約変更なしで接続可能 |
| NS-PLT-006 | Evidence Pack v1定義 | P2 | M | NS-CLD-007 | 監査証跡・逸脱記録・変更履歴を含む提出用テンプレートを作成 |
...
| NS-EDG-004 | NetworkTask ingest-sensor送信実装 | P0 | M | NS-EDG-003 | HTTPSで`POST /functions/v1/ingest-sensor`へ冪等送信 |
...
| NS-CLD-004 | ingest-movement実装 | P0 | M | NS-CLD-001 | JWT検証と`ENTER/EXIT`/`CARE_*`の正規化登録が成功 |
| NS-CLD-005 | notify-staff実装 | P0 | M | NS-CLD-003 | `SITTING/OUT`でFCM/APNs送信処理が起動 |
| NS-CLD-006 | Realtime publication設定 | P0 | S | NS-CLD-001 | `sensor_events/staff_movements/care_records`のINSERT購読可 |
| NS-CLD-007 | Evidence SQLとPDF生成実装 | P0 | L | NS-CLD-003,NS-CLD-004 | 日次レポートをPDFで出力可能 |
| NS-CLD-008 | profiles/auth bootstrap実装 | P0 | M | NS-CLD-001 | Staff/Adminログインとプロフィール参照可 |
...
2. Cloud基盤: `NS-CLD-001` -> `NS-CLD-002` -> `NS-CLD-008` -> `NS-CLD-003/004`
3. Edge/App並行:
...
※ 実行順の確定版は `docs/specs/93_実装着手順_確定版.md` を正本とする。
